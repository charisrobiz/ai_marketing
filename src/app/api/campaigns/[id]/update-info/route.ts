...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const oldInfo = JSON.parse(campaign.product_info as string);
  const settings = getSettings();

  // DB 업데이트
  db.prepare('UPDATE campaigns SET product_info = ?, updated_at = datetime(\'now\') WHERE id = ?')
    .run(JSON.stringify(newInfo), campaignId);
  ...
  // 본부장 하나의 변경 영향도 분석
  addEvent(campaignId, 'hana', '하나', 'system', `캠페인 정보 변경이 감지되었습니다. 변경 영향도를 분석합니다.`);
  addEvent(campaignId, 'hana', '하나', 'chat', `변경 사항: ${changes.join(' / ')}`);
...
{"level":"partial","reason":"타겟 고객이 변경되어 카피라이팅 톤 조정 필요","affected_areas":["카피라이팅","타겟팅 광고"],"recommendation":"공감형/바이럴형 카피를 새 타겟에 맞게 수정하고, 퍼포먼스 광고 타겟 세그먼트를 재설정"}`;
  ...
  if (impactResult.level === 'minor') {
    addEvent(campaignId, 'hana', '하나', 'chat', `분석 결과: 단순 변경입니다. ${impactResult.reason}`);
    addEvent(campaignId, 'hana', '하나', 'system', `✅ 기존 마케팅 전략 유지. 변경 사항을 팀에 공유합니다.`);
    addEvent(campaignId, 'minseo', '민서', 'chat', `확인했습니다. 기존 플랜 그대로 진행하겠습니다.`);

  } else if (impactResult.level === 'partial') {
    addEvent(campaignId, 'hana', '하나', 'chat', `분석 결과: 일부 조정이 필요합니다. ${impactResult.reason}`);
    addEvent(campaignId, 'hana', '하나', 'system', `📝 긴급 내부 미팅을 소집합니다. 영향 범위: ${impactResult.affected_areas.join(', ')}`);

    // 내부 회의
    addEvent(campaignId, 'hana', '하나', 'chat', `팀 여러분, 캠페인 정보가 변경되었습니다. 영향받는 영역만 빠르게 수정합시다.`);
    addEvent(campaignId, 'hana', '하나', 'chat', `추천 방향: ${impactResult.recommendation}`);

    if (impactResult.affected_areas.includes('카피라이팅')) {
      addEvent(campaignId, 'jiwoo', '지우', 'chat', `변경된 정보 확인했어요. 기존 카피 중 영향받는 부분만 수정하겠습니다. 전체 재작업은 필요 없을 것 같아요.`);
      addEvent(campaignId, 'minseo', '민서', 'chat', `지우님 동의합니다. 위너 소재의 톤은 유지하되, 변경된 부분만 반영해주세요.`);
    }
    if (impactResult.affected_areas.includes('비주얼') || impactResult.affected_areas.includes('비주얼 컨셉')) {
      addEvent(campaignId, 'yuna', '유나', 'chat', `비주얼 가이드라인 중 해당 부분만 업데이트할게요. 전체 무드는 유지합니다.`);
    }
    if (impactResult.affected_areas.includes('타겟팅 광고') || impactResult.affected_areas.includes('타겟팅')) {
      addEvent(campaignId, 'taeyang', '태양', 'chat', `타겟 세그먼트 조정이 필요하면 바로 반영하겠습니다. 기존 데이터 기반으로 빠르게 A/B 테스트 돌릴 수 있어요.`);
      addEvent(campaignId, 'eunji', '은지', 'chat', `기존 성과 데이터와 비교 분석해서 새 타겟의 예상 CPA도 산출해드릴게요.`);
    }
    if (impactResult.affected_areas.includes('SEO') || impactResult.affected_areas.includes('SEO 키워드')) {
      addEvent(campaignId, 'jiwoo', '지우', 'chat', `SEO 키워드도 재점검할게요. 기존 인덱싱된 콘텐츠는 유지하면서 새 키워드를 추가합니다.`);
    }

    addEvent(campaignId, 'hana', '하나', 'chat', `좋습니다. 각자 담당 영역 수정 진행해주세요. 수정 완료 후 재검토하겠습니다.`);
    addEvent(campaignId, 'hana', '하나', 'system', `📋 일부 수정 진행 중. 수정 완료 후 본부장 재검토 예정.`);

  } else {
    // major
    addEvent(campaignId, 'hana', '하나', 'chat', `분석 결과: 전면 재수립이 필요합니다! ${impactResult.reason}`);
    addEvent(campaignId, 'hana', '하나', 'system', `🚨 긴급 전체 회의 소집! 마케팅 전략 전면 재검토가 필요합니다.`);

    // 긴급 전체 회의
    addEvent(campaignId, 'hana', '하나', 'chat', `전원 집중해주세요. 핵심 정보가 크게 바뀌어서 기존 전략을 그대로 가져갈 수 없습니다.`);
    addEvent(campaignId, 'minseo', '민서', 'chat', `본부장님, 변경 내용 확인했습니다. 30일 플랜을 새 정보 기반으로 재수립해야 할 것 같아요. 기존 1-2주차 데이터는 참고용으로 활용하겠습니다.`);
    addEvent(campaignId, 'jiwoo', '지우', 'chat', `카피라이팅 전면 재작업 들어갈게요. SEO 키워드도 완전히 다시 잡아야 합니다.`);
    addEvent(campaignId, 'yuna', '유나', 'chat', `비주얼 컨셉도 바뀌어야 해요. 새 제품 포지셔닝에 맞는 무드보드부터 다시 시작합니다.`);
    addEvent(campaignId, 'doha', '도하', 'chat', `기존 숏폼 소재는 폐기하고, 새 컨셉으로 재제작할게요.`);
    addEvent(campaignId, 'taeyang', '태양', 'chat', `광고 캠페인도 전면 재설정합니다. 기존 데이터에서 학습한 건 타겟 반응 패턴 정도만 참고할 수 있어요.`);
    addEvent(campaignId, 'eunji', '은지', 'chat', `기존 투표/성과 데이터는 아카이브하고, 새 기준으로 분석 대시보드를 리셋하겠습니다.`);
    addEvent(campaignId, 'siwon', '시원', 'chat', `파이프라인 재가동 준비 완료입니다. 본부장님 지시 대기합니다.`);
    addEvent(campaignId, 'hana', '하나', 'chat', `모두 감사합니다. 새 정보 기반으로 전략 재수립 → 소재 재생성 → 투표 → 승인 순으로 진행합니다. CEO님께도 보고드리겠습니다.`);
    addEvent(campaignId, 'hana', '하나', 'system', `🔄 전면 재수립 착수. 새 엔진 실행이 필요합니다.`);
  }
